	...
	SubShader {
		...
		Pass {
			...
			half4 frag(vertexOutput i) : COLOR
			{
				...
				half3 local_coords_1 = half3 (2.0 * normal_map_1.rg - half2(1.0, 1.0), 1.0);
					
				half4 normal_map_2 = tex2D(_BumpMap, -water_movement + normal_mapping);
				half3 local_coords_2 = half3 (2.0 * normal_map_2.rg - half2(1.0, 1.0), 1.0);

				half3 local_coords_final = local_coords_1.xyz * local_coords_2.xyz;
				
				// Normal transpose matrix
				half3x3 local_2_world_transpose = half3x3 (i.tangent_world.xyz, i.binormal_world.xyz, i.normal_world.xyz);
				...
				half3 specular_reflection =  lerp (10 , 0, foam_spread_1) * diffuse_reflection.rgb * _SpecColor.rgb * pow (max (0.0, dot (reflect(-i.light_direction.xyz, normal_direction.xyz), i.view_direction.xyz)), _Shininess);
				...
				// Reflection
				half3 reflect_dir = reflect (i.view_direction, half3(0,0,0));
				...
				return half4 (lerp (1.5 * textures_final.rgb * lighting_final.rgb, cube_map.rgb, _ReflectPow), final_color.a);
			}
