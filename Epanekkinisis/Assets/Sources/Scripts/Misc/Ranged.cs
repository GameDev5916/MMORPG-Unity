...
public class Ranged : MonoBehaviour {
	...
	void Start () {
		...
		RunDistance = AttackDistance + BackDistance;
	...
	void Update () {
		...
		if(!GetComponent<Patrol>().patrol){
			...
			if((ClosestPlayerDistance > BackDistance && ClosestPlayerDistance < RunDistance) && !GetComponent<AnimationManager>().animationComp.isPlaying && !Physics.Raycast(transform.localPosition, ClosestPlayer.transform.localPosition - transform.localPosition, out hit, Vector3.Distance(transform.localPosition, ClosestPlayer.transform.localPosition)-1)){
				
				gameObject.SendMessage("CustomAnimation", "attack");
				
			}
			
			if(ClosestPlayerDistance < BackDistance && !Physics.Raycast(transform.localPosition, ClosestPlayer.transform.localPosition - transform.localPosition, out hit, Vector3.Distance(transform.localPosition, ClosestPlayer.transform.localPosition)-1)){
				
				if(!GetComponent<AnimationManager>().animationComp.isPlaying){
					gameObject.SendMessage("CustomAnimation", "back");
				}
			...
			if(ClosestPlayerDistance > AttackDistance || (ClosestPlayerDistance < AttackDistance && Physics.Raycast(transform.localPosition, ClosestPlayer.transform.localPosition - transform.localPosition, out hit, Vector3.Distance(transform.localPosition, ClosestPlayer.transform.localPosition)))){
